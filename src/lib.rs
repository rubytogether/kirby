  ...
  for (time, names) in right {
    ...
    for (name, versions) in names {
      ...
      for (version, count) in versions {
        let left_count = left_versions.entry(version.to_owned()).or_insert(0);
        *left_count += count;
      }
    }
  }
...
fn increment(counters: &mut NameMap, name: FieldName, value: &str) {
  let count = counters[name].entry(value.to_owned()).or_insert(0);
  *count += 1;
}
...
pub fn print_unknown_user_agents(path: &str, opts: &Options) {
  ...
  while file::reader(&path, &opts)
    ...
    > 0
  {
    let r: request::Request = serde_json::from_slice(line.as_bytes()).unwrap();
...
pub fn stream_stats(mut stream: Box<BufRead>, opts: &Options) -> TimeMap {
